...
Proof is the dump containing `text="Gesture actions"` and the full `Select actions to train` prompt after a tap on `Gestures` on Pen settings, plus the screenshot.
