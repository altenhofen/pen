...
Calibration is the training minigame behind `Calibrate` on Pen settings. It first shows a grid of `0-9`, `A-Z`, and `a-z`. The user selects glyphs, then taps `Start`. The writing canvas fills the screen and shows the current character as a large watermark. The user may draw any number of examples. `Next` splits that ink into separate samples and moves on. That canvas is not the IME.
...
- **Read the prompt.** The screen shows `Now writing 0`. `artifacts/calibration/hierarchy.xml` contains that exact `text=` value.
