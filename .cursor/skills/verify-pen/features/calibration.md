...
Calibration is the training minigame behind `Calibrate` on Pen settings. It first shows a grid of `0-9`, `A-Z`, and `a-z`. The user selects glyphs, then taps `Start`. The writing canvas fills the screen and shows the current character as a large watermark. The user may draw any number of examples. `Next` splits that ink into separate samples, saves that glyph's samples right away, and moves on. Saved samples add to earlier calibrations for the same glyph instead of replacing them. `Cancel` keeps every glyph already confirmed with `Next`. Rotating the screen keeps the session. That canvas is not the IME.
