...
- Launcher activity `io.github.altenhofen.pen/.MainActivity` shows `Hello Android!`.
...
- Prefer the visible text `Hello Android!` over coordinates.
...
- Record the feature ID (`greeting` or `ink-keyboard`) with every artifact.
...
- [Greeting](./greeting.md) covers the launcher activity and the text `Hello Android!`.
