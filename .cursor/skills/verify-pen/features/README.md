...
- Record the feature ID (`settings`, `calibration`, `character-fine-tune`, `profile-export`, `profile-import`, or `ink-keyboard`) with every artifact.
...
- [Settings](./settings.md) covers the launcher activity, the three motor sliders, `Calibrate 8 and 9`, `Fine-tune 0-9 and a-z`, `Export profile`, and `Import profile`.
